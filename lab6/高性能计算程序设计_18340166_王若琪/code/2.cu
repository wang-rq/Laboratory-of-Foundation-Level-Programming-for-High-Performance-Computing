...
int main(int argc, char **argv)
{
	...
	int OpenMp_THREAD_NUM = atoi(argv[5]);
...
#pragma omp parallel private(cuda_a, cuda_b, cuda_c)
	{
		...
		cudaMemcpy(cuda_a + OpenMp_thread_id * M * N / OpenMp_THREAD_NUM, sub_a, sizeof(float) * M * N / OpenMp_THREAD_NUM, cudaMemcpyHostToDevice);
		cudaMemcpy(cuda_b, b, sizeof(float) * N * K, cudaMemcpyHostToDevice);

		// 在CUDA 中执行函数
		matMultCUDA<<<blocks_num, BLOCK_SIZE, 0>>>(cuda_a + OpenMp_thread_id * M * N / OpenMp_THREAD_NUM, cuda_b, cuda_c, M / OpenMp_THREAD_NUM, N, K, BLOCK_SIZE);

		//cudaMemcpy 将结果从显存中复制回内存
		cudaMemcpy(sub_c, cuda_c, sizeof(float) * M * K / OpenMp_THREAD_NUM, cudaMemcpyDeviceToHost);
	}
