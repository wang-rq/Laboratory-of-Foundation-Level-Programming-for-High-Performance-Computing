/******************************************
 * Compile:
 * gcc -o heated_plate_openmp heated_plate_openmp.c -fopenmp
 * Run:       
 * ./heated_plate_openmp <number of threads>
 ******************************************/
...
*/
#pragma omp parallel shared(mean, w) private(i, j) num_threads(ThreadNumber)
  {
#pragma omp for
    for (i = 1; i < M - 1; i++)
    {
...
*/
  ...
  while (epsilon <= diff)
  {
#pragma omp parallel shared(u, w) private(i, j) num_threads(ThreadNumber)
    {
/*
...
*/
    diff = 0.0;
#pragma omp parallel shared(diff, u, w) private(i, j, my_diff) num_threads(ThreadNumber)
    {
      my_diff = 0.0;
#pragma omp for
      for (i = 1; i < M - 1; i++)
